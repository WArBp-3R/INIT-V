"""
The Calculator module which contains the Calculator class.
"""
...
class Calculator:
    ...
    highest_protocols: A set of protocol names of the protocols which appear on the highest layer
    in the packet(s).
    """
    ...
    def __init__(self, pcap_path: str):
        ...
        :param: pcap_path: Path of the PCAP file which is going to be used to base the calculations
        on.
        """
        ...
        self._connection_protocol_packets: dict[Connection, dict[str, list[Packet]]] = {}
        self._connection_oldest_newest_packets: dict[Connection, (Packet, Packet)] = {}
        self._connection_oldest_newest_protocol_packets: dict[Connection,
                                                              dict[str, (Packet, Packet)]] = {}
        self._connection_packets: dict[Connection, list[Packet]] = {}
        self._connection_statistics: dict[Connection, dict[str, str]] = {}
        self._connection_statistics_per_protocol: dict[Connection, dict[str, dict[str, str]]] = {}
        self._sent_received_packet_count: dict[str, (int, int)] = {}
        self._protocols_use_count: dict[str, int] = {}
        # Private parameter just for testing, some test assertions are skipped if some packets were
        # skipped.
    ...
    def calculate_topology(self) -> NetworkTopology:
        ...
        return NetworkTopology(list(self._devices.values()), list(self._all_connections),
                               self.protocols, self.highest_protocols)
    ...
    def calculate_run(self, config: Configuration) -> RunResult:
        ...
        if config.autoencoder:
            autoencoder_history, autoencoder_packet_mapping = self.backend_adapter.\
                calculate_autoencoder(config)
        ...
        return RunResult(timestamp, config, MethodResult(pca_result, autoencoder_result),
                         PerformanceResult(pca_performance,
                                           autoencoder_history.history if autoencoder_history
                                           else {}))
        ...
        for device_mac, connections_per_protocol in connections.items():
            ...
            for protocol, protocol_connected_devices in connections_per_protocol.items():
                for connected_device in protocol_connected_devices:
                    if connected_device not in connected_devices:
            ...
            for connected_device, protocols in connected_devices.items():
                if connected_device not in self._connections:
                    if device_mac not in self._connections:
                        self._connections[device_mac] = {}
                    new_connection = Connection(self._devices[device_mac],
                                                self._devices[connected_device], set(protocols),
                                                {}, {})
    ...
    def _sort_packets(self):
        ...
        _protocols_use_count: A dictionary mapping a protocol name to an integer which stands for
        the usage count of that protocol.
        _sent_received_packet_count: A dictionary mapping a MAC address to a tuple of integer
        values, first value of the tuple is the total amount of packets sent by the device, the
        second value is the total amount of packets
        received by the device.
        _connection_protocol_packets: A dictionary mapping a connection object to another
        dictionary. The value dictionary maps protocol names to a list of packets which contain a
        layer using that protocol.
        _connection_packets: A dictionary mapping a connection object to a list of packets which
        were sent between the devices of that connection.
        """
        for packet, protocols in self._packets:
            ...
            if protocols[-1] not in self._protocols_use_count:
            ...
            self._sent_received_packet_count[sender_mac] = (self._sent_received_packet_count
                                                            [sender_mac][0] + 1,
                                                            self._sent_received_packet_count
                                                            [sender_mac][1])
            self._sent_received_packet_count[receiver_mac] = (self._sent_received_packet_count
                                                              [receiver_mac][0],
                                                              self._sent_received_packet_count
                                                              [receiver_mac][1] + 1)
            # Step 1: Getting the correct connection object according to the src/dst mac address of
            # the packet:
            try:
                packet_connection = self._connections[sender_mac][receiver_mac]
            except KeyError:
                packet_connection = self._connections[receiver_mac][sender_mac]
            # Step 2: initializing dictionary entries for the connection and protocols if there was
            # no packet of
            # that connection and protocol processed yet.
            if packet_connection not in self._connection_protocol_packets:
                self._connection_protocol_packets[packet_connection] = {}
            if packet_connection not in self._connection_packets:
                self._connection_packets[packet_connection] = []
            for layer in protocols:
                self.protocols.add(layer)
                if layer not in self._connection_protocol_packets[packet_connection]:
                    self._connection_protocol_packets[packet_connection][layer] = []
    ...
    def _parse_connection_statistics(self):
        ...
        for connection in self._all_connections:
            self._connection_statistics[connection] = {}
            self._connection_statistics_per_protocol[connection] = {}
            ...
            self._connection_statistics[connection]["Packet Count"] = \
                str(len(self._connection_packets[connection]))
            for protocol, protocol_packets in self._connection_protocol_packets[connection].items():
                ...
                if connection not in self._connection_oldest_newest_protocol_packets:
                    self._connection_oldest_newest_protocol_packets[connection] = {}
                self._connection_oldest_newest_protocol_packets[connection][protocol] \
    ...
    def _calculate_packets_per_second(self):
        """
        Calculates the packets per second for each connection and for each protocol in that
        connection.
        """
        for connection in self._all_connections:
            ...
            for protocol in self._connection_protocol_packets[connection]:
                protocol_packet_count: int = int(self._connection_statistics_per_protocol
                                                 [connection][protocol]["Packet Count"])
                protocol_packet_total_time: timedelta = datetime.fromtimestamp(
                    ...
                    - datetime.fromtimestamp(self._connection_oldest_newest_protocol_packets
                                             [connection][protocol][0].time)
                self._connection_statistics_per_protocol[connection][protocol]\
    ...
    def _update_connection_information(self):
        """
        Updates the statistic relevant values of the connections according to the values obtained
        by the previous
        calculations.
        """
        for connection in self._all_connections:
            ...
            for stat_name, stat_value in connection_statistics.items():
                ...
                for protocol, protocol_statistics in self._connection_statistics_per_protocol[
                        connection].items():
                    if protocol not in connection.protocol_connection_information:
    ...
    def _parse_method_result(self, mapped_packets: list[(float, float)]) \
        ...
        """
        method_result: list[(float, float, str, str)] = []
        for packet_mapping, (packet_information, packet_protocols) in \
                zip(mapped_packets, self._packets):
            protocol_timestamp \
                = datetime.fromtimestamp(float(f"{packet_information.time:.6f}")).isoformat(sep=" ")
            packet_dict: dict[str, str] \
                = {"Highest protocol": packet_protocols[-1], "Timestamp": protocol_timestamp} \
    ...
    def _calculate_figures(self):
        ...
        """
        packets_sent_received_data = dict({"Packets sent": [], "Packets received": [],
                                           "mac address": []})
        ...
        for connection in self._all_connections:
            ...
            connections_throughput_data["Connection"].append(
                str(connection.first_device.mac_address) + "-" +
