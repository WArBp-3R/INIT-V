...
from backend.BackendInterface import BackendInterface
...
from backend.NetworkPacketAutoencoder import NetworkPacketAutoencoder
...
class Backend(BackendInterface):
    """Class representing the ML backend. Implements the BackendInterface"""
    pcaps = {}
    ...
    def get_connections(self, pcap_id: str) -> dict:
        ...
        for pkt in packets:
            ...
            for layer in get_packet_layers(pkt):
                if layer in res[src].keys() and not dst in res[src][layer]:
                    res[src][layer].append(dst)
                else:
                    res[src][layer] = [dst]
                if layer in res[dst].keys() and not src in res[dst][layer]:
